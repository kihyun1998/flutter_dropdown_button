...
* **FIX**: the same was true of an **ordinary close**, which is the general case and was found by the completeness pass on the first fix. `isOpen` is the overlay entry's existence, so it stays true for the whole reverse tween — measured, a row tapped 150ms into the default 200ms close, at 38% opacity, still selected. Dismiss the menu and you could still be given the value you had just cancelled; tapping a second row while the first tap's close played out selected twice. Both now refuse
...
* **CHANGE**: a disabled row now carries `enabled: false` in the semantics tree rather than merely losing its tap action. Dropping the action alone left a node still announcing a chosen state and still taking focus while saying nothing about being a control — half a node, which `docs/adr/0001` rule 3 exists to forbid
