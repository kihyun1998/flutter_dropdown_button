...
* **BREAKING**: Raised the SDK floor to Flutter `>=3.27.0` / Dart `>=3.6.0`. It claimed `>=3.10.0`, but `lib/` has used `WidgetStateProperty` since 2.0.0 and `Color.withValues` since a later refactor — 3.22 and 3.27 APIs respectively. `pub` resolved against the promise, so a project on 3.10 installed the package and then failed to compile inside our source. Not a new restriction; an honest statement of an old one
