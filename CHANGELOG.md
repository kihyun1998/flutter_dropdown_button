...
* **CHANGE**: on by default, for every dropdown that scrolls. The destination of a notch is unchanged; only the path to it is. It applies to a mouse wheel on any platform, an Android device's included. Trackpads, dragging, touch, the scrollbar thumb, the keyboard, `scrollToSelectedItem` and the reset on a new search query behave as before — except in Firefox, which reports a trackpad as a mouse
...
* **CHANGE**: requires `flutter_smooth_wheel_scroll` `^0.1.3`. 0.1.2 animated trackpad scrolling on the web too, every small delta retargeting the motion, because the browser sends it as a scroll signal of kind `trackpad` and 0.1.2 never looked at the kind (#161)
...
* **DOCS**: the example gains a **Wheel scrolling** recipe: the default glide next to the old jump, with every motion on its knobs and the `wheelMotion:` line that produces it (#159)
...
* **TEST**: `wheel_scroll_test.dart` sends real wheel events to an open menu. Discriminating power confirmed by mutation: a controller that jumps reddens the glide test, a 400 ms default reddens the 300 ms settle test, and assigning the motion only when the controller is created reddens both "motion changed" tests. It also pins a search query mid-glide (reddens without the reset to the top), closing or removing the dropdown mid-glide, and a notch past the end of a gliding menu reaching the page around it (reddens when upstream stops passing it outward), and a trackpad scroll signal jumping (reddens against `flutter_smooth_wheel_scroll` 0.1.2)
