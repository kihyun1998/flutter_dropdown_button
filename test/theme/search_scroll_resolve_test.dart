...
void main() {
  group('search field', () {
    ...
    test('a themed border wins over the ambient one', () {
      ...
      final enabled =
          theme.resolve(ambient).decoration.enabledBorder!
    ...
    test('the field is only filled when a background is given', () {
      expect(
        const SearchFieldTheme().resolve(ambient).decoration.filled,
        false,
      );
      expect(
        const SearchFieldTheme(
          backgroundColor: red,
        ).resolve(ambient).decoration.filled,
        true,
      );
    });
  ...
  group('scrollbar', () {
    ...
    test(
      'unnamed slots stay null, so an ambient ScrollbarTheme keeps its say',
      () {
    ...
    test(
      'the ScrollbarTheme is only overridden when the caller names a slot',
      () {
        expect(
          const DropdownScrollTheme(
            radius: Radius.circular(4),
          ).resolve().overridesScrollbarTheme,
          isFalse,
