/// A dropdown with no button box — the caller draws the anchor.
library;
...
class BareAnchorRecipe extends StatefulWidget {
  const BareAnchorRecipe({super.key});

  @override
  State<BareAnchorRecipe> createState() => _BareAnchorRecipeState();
}

class _BareAnchorRecipeState extends State<BareAnchorRecipe> {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 32),
              Text('Custom items, bare', style: theme.textTheme.labelLarge),
              const SizedBox(height: 8),
              _SearchField(
                fieldKey: _customFieldKey,
                // The default constructor, not `.text()`: rows are whatever
                ...
                scope: FlutterDropdownButton<String>(
                  ...
                  value: _field,
                  onChanged: (f) => setState(() => _field = f!),
                  positioningKey: _customFieldKey,
                  itemBuilder: (field, isSelected) => Padding(
