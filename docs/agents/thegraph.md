...
| `flutter_example_template` — the gallery `example/` is built on | the shell's ports and their contracts: `ShellDestinations`, `StageDestination`/`RouteDestination`, `PreviewStage`, the Code pane | the real source in the pub cache at the pinned version — raw, never pub.dev's rendered docs | **binding** — a published package this repo depends on, and its undocumented contracts bind whether or not they are written down. Read at **0.2.0**. Two such contracts have been found and both are now written down upstream: a roster needs no `StageDestination` (0.1.0 threw without one, [template#10](https://github.com/kihyun1998/flutter_example_template/issues/10)), and an empty `ShellCategory` draws nothing (0.1.0 drew a header over *nothing here yet*, [template#11](https://github.com/kihyun1998/flutter_example_template/issues/11)). |
