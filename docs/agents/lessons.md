...
- **#95 (가장 강한 RED 는 새 테스트가 아니라 우회 분기의 삭제였다).**
  `overlay_lifecycle_test.dart` 와 `multi_select_test.dart` 에 `if (!betaMenuOpen())
  { 한 번 더 탭 }` 이 있었고 주석까지 달려 있었다 — *"배리어 때문에 두 번 걸릴 수 있다"*.
  옆에 새 테스트를 세우는 대신 **그 분기를 지웠다.** 지우자마자 둘 다 빨개졌고
  (`Expected: true / Actual: <false>`), 픽스 후 초록. 버그를 우회하려고 누군가 손으로
  쓴 코드가 그 버그의 가장 정확한 명세다 — 새로 지어낸 시나리오와 달리 **실제로 아팠던
  자리**라는 증거가 이미 붙어 있다.
...
- **#95 (텍스트 상자는 탭 영역이 아니다 — `:156` 규칙의 두 번째 사례).** "메뉴 자기
  패딩을 탭하면 배리어로 떨어진다" 를 증명하려고 `getRect(find.text('Alpha 1'))` 의
  `top - 3` 을 탭했고, 메뉴가 닫혔으므로 증명됐다고 이슈에 보고했다. **틀렸다.** 행의
  탭 영역은 `(9,55,157,103)` 인데 텍스트 상자는 `(25,69,124.8,89)` — `top-3=66` 은
  패딩이 아니라 **행 안쪽**이었다. 메뉴가 닫힌 건 배리어가 아니라 **항목이 선택돼서**고,
  `onChanged` 스파이를 붙이자 `picked=Alpha 1` 이 나왔다. 그 위에 "가드가 필수" 라는
  설계 결론을 하나 세워둔 뒤였다. **닫혔다/false 는 결과가 아니라 질문이다 — 무엇이
  닫았는지 같이 재지 않으면 아직 측정이 아니다.** 히트 경로를 찍었으면 배리어가
  index 17 이라는 게 바로 보였다.
...
- **#143 (스크린샷은 측정이 아니다 — 유령 하나가 이슈와 코드 주석까지 갔다).** Flutter web
  은 canvas 에 그리므로 DOM 이 없다. 그래서 확인 수단이 스크린샷뿐이라고 판단하고 헤드리스
  크롬 + CDP 로 찍어 **읽었다**. "버튼이 318 이 아니라 전체 폭에 400px 높이로 그려진다" 로
  읽혔고, 원인을 못 찾자 **"설명되지 않음"** 을 이슈 코멘트와 레시피 주석 양쪽에 남겼다.
  다음 세션에서 `tester.getSize()` 로 재니 **318.0 × 50.0** — 평범한 `Scaffold` 에서도,
  같은 1600×1000 뷰포트의 실제 shell 에서도. 즉 **관찰 자체는 맞았고 대상이 틀렸다**: 402px
  는 그 **직전 빌드**의 것이었다(고친 뒤 다시 빌드는 했지만 다시 찍지는 않았다). "더 높고
  **더 넓다**" 도 정확했다 — `expand` 는 `fillsWidth` 도 켜서 내용 Row 를 `MainAxisSize.max`
  로 만들기 때문에 `Column` 안에서 양방향을 다 채운다(#145 에서 문서까지 고쳤다). 틀린 것은
  결론뿐이다: "설명되지 않는 잔여" 가 아니라 그냥 **낡은 빌드**였다. `getSize` 는 숫자를
  돌려주고 스크린샷은 **해석해야 할 그림**을 돌려준다 — 그리고 그림에는 찍힌 시각이 안 적혀
  있다. **캔버스라 DOM 이 없다는 건 위젯 트리를 못 잰다는 뜻이 아니다**: 레이아웃은
  `flutter test` 안에 그대로 있고, 402 라는 숫자도 결국 거기서 나왔다.
- **#145 (어떤 구현으로도 못 틀리는 단언은 게이트가 아니다).** "tight 부모가 `width` 를
  이긴다" 를 **바깥 위젯**의 크기로 단언했다. 초록이었고, 뮤테이션을 걸어도 초록이었다 —
  당연하다. Flutter 에서 자식 render box 는 들어온 tight 제약을 **반드시** 만족하므로,
  그 단언은 이 패키지에 대한 주장이 아니라 **프레임워크에 대한 주장**이고 어떤 구현으로도
  빨개질 수 없다. 실제로 틀릴 수 있는 대상은 **칠해지는 상자**(decoration 을 든
  `Container`)다 — 위젯이 내부에서 스스로 제약을 풀면 전체 폭 슬롯 안에 260 짜리 버튼이
  그려지고, 그게 바로 문서가 권하는 `Align` 을 불필요하게 만드는 대안 설계다. 단언을 거기로
  옮기니 뮤테이션이 빨개졌다. **"이 단언을 빨갛게 만드는 구현이 존재하는가" 를 못 답하면
  그건 커버리지가 아니라 장식이다.**
- **#143 (못 찾았다를 "없다" 로 승격시켰다).** `_applyWidthConstraints` 를 읽고 "`widget.width`
  가 적용되는 곳을 못 찾았다" 를 이슈에 남겼는데, 같은 파일 **한 메서드 앞** `:415` 에
  `Container(width: widget.width, …)` 로 그냥 있었다. `firsthand` 의 "미확인은 부재가 아니라
  공백" 이 정확히 이 모양이다 — 공백으로 적었으면 다음 사람이 5초에 메웠고, 부재로 적혔기에
  존재하지 않는 버그의 재현 절차가 한 페이지 붙었다.

## Step 5 — 적대적 검증 (서로 다른 렌즈)

- **#95 (이슈에 적힌 설계 초안은 승인됐어도 가설이다).** 오너가 처방 B 를 고르며 같이
  실려 있던 초안이 *"배리어의 `hitTest` 에서 형제 트리거의 **사각형** 안이면 거절"* 이었다.
  렌즈 둘이 독립적으로 이걸 깼고, 실측이 다섯 구성에서 어긋났다 — 클립·회전·
  `IgnorePointer`·`Offstage`·모달. 최악은 **다이얼로그 뒤 드롭다운의 rect 가 안 가려졌을
  때와 바이트 단위로 같다**는 것: 어떤 기하학적 정직성 검사도 통과하고, 구멍을 뚫으면
  탭이 **다이얼로그를 닫는다.** 뿌리는 순서였다 — `hitTest` 는 경로 index 0 에서
  판단하는데 대상 트리거는 index 10 에 들어온다. **판단 시점에 답이 없으면 기하학으로
  복원하게 되고, 기하학은 클립·변환·가림을 못 본다.** 처방(무엇을)은 오너 것이고
  메커니즘(어떻게)은 렌즈가 뒤집을 수 있다 — 둘을 한 덩어리로 승인된 걸로 읽지 말 것.

- **#95 (렌즈의 정정도 후보지 판정이 아니다).** 반박 렌즈가 *"브리프가 이미 철회된
  측정을 인용했다"* 며 `lessons.md:149-162` 를 근거로 들었다. 열어보니 철회된 건 **G2
  (패키지가 심은 surface)** 프로브였고, 브리프가 쓴 **E2 (bare 호스트)** 숫자와 순환성
  논거는 `:157-162` 에 그대로 살아 있었다. 렌즈가 둘을 합쳤다. 받아들였으면 멀쩡한
  기각 근거를 되살릴 뻔했다. **렌즈가 나를 정정할 때도 원문을 열어보고 받는다.**

- **#95 (게이트가 절대 못 잡는 조건 — `PointerDeviceKind.touch`).** 프로토타입이 veto 를
  `kind == touch` 로 좁혀놨었다. 위젯 테스트는 기본이 touch 라 **전 스위트가 초록인데
  데스크톱·웹에서는 아무 일도 안 한다** (`dismisses=1 bTaps=0`). analyze 도 커버리지도
  못 본다 — 코드는 전부 실행되고 결과만 틀리다. 포인터 종류로 분기하는 조건은 반드시
  `kind: PointerDeviceKind.mouse` 테스트를 같이 세운다.
...
- **#95 (도달 불가능한 방어 분기는 100% 바닥이 영원히 못 채운다).** `handleEvent` 에
  `final result = _results[entry]; if (result == null) return;` 을 썼는데, 엔트리는
  디스패치 내내 살아 있으므로 그 `return` 은 **도달 불가능**하다 — 즉 커버리지 리포트에
...
- **#143 (게이트 전부 초록인데 레시피는 매 빌드 throw 하고 있었다).** `flutter analyze`,
  테스트 11 개, contract 테스트 둘, API 커버리지 **100%** 가 모두 초록인 채로 — dismissal
  레시피는 `Competing ParentDataWidgets` 를 **매 빌드 던지고** 있었고(`Expanded` 로 감싼
  자식이 `expand: true` 라 `Expanded` 가 둘), multi-select 은 `Column` 안에서 402px 로
  늘어나 있었으며, 그 뒤 넣은 `width: 260` 은 `ListView` 의 **tight** cross-axis 제약에
  먹혀 1392 로 그려지고 있었다. 원인은 하나다: **아무 테스트도 레시피를 pump 하지 않았다.**
  `shell_renders_test` 는 메뉴 라벨만 읽는데 라벨은 roster 에서 그려지므로 stage 가 던져도
  거기 그대로 있고, contract 테스트는 레시피를 **텍스트로만** 읽는다. `recipe_renders_test.dart`
  가 roster 를 돌며 pump 하고 예외·늘어난 높이·도달 못 한 width 를 잡는다 — 생기자마자 셋 다
  잡았다. **인자를 셌다는 주장과 그려졌다는 주장은 다른 주장이다.**
- **#143 (`Expanded` 와 `width` 는 같은 부모에서 서로 다른 이유로 죽는다).** `ListView` 안의
  드롭다운에서 `expand` 는 `Expanded` 가 `FlexParentData` 를 쓸 `Flex` 가 없어서 죽고,
