...
- **#143 (스크린샷은 측정이 아니다 — 유령 하나가 이슈와 코드 주석까지 갔다).** Flutter web
  ...
  같은 1600×1000 뷰포트의 실제 shell 에서도. 스크린샷 속 "전체 폭 테두리 상자" 는 stage 의
  **device frame** 이었고, 진짜 402px 는 그 **직전 빌드**의 것이었다(고친 뒤 다시 빌드는
  했지만 다시 찍지는 않았다). `getSize` 는 숫자를 돌려주고 스크린샷은 **해석해야 할 그림**을
  돌려준다. **캔버스라 DOM 이 없다는 건 위젯 트리를 못 잰다는 뜻이 아니다** — 레이아웃은
